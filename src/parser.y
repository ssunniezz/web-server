...
request_headers: request_headers request_header{
	if (!(parsing_request->header_count & (parsing_request->header_count-1)))
		parsing_request->headers = 
			(Request_header *) realloc(parsing_request->headers, sizeof(Request_header)*2*parsing_request->header_count);
} | {}; 
